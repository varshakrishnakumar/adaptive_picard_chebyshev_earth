...
#define debug_grav_itr 0
 ...
 void perturbed_gravity(double t, double* Xo, double err, int i, int M, double deg, double deg_low, int hot, double* G, double tol, int* itr, double* Feval){
...
//      Grav_Approx(t,Xo,G,Feval);
     grav_model = 0;
     Grav_Full(t,Xo,G,tol,deg,Feval,deg_low,grav_model);
     if (i==M+1){
        ITR1  = *itr;
        ITR2  = *itr;
        ITR3  = *itr;
...
//      Grav_Approx(t,Xo,Gapprox,Feval);
     ...
     Grav_Full(t,Xo,G,tol,deg,Feval,deg_low,grav_model);
     grav_model = 0;
     Grav_Full(t,Xo,Gapprox,tol,deg,Feval,deg_low,grav_model);
     for (int j=0; j<=2; j++){
